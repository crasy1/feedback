...
public sealed class FeedbackDetailAndCommentTests(IntegrationTestFixture fixture)
{
    ...
    {
        // 两个玩家必须落在同一个游戏里：越权读取要测的是"别人的反馈"，不是"别的游戏"。
    ...
    private async Task<int> CreateFeedbackAsync(HttpClient client)
    {
        var response = await client.PostAsJsonAsync(Game.FeedbackPath, new Dictionary<string, object?>
        {
    ...
    public async Task Owner_gets_detail_with_comments()
    {
        ...
        await owner.PostAsJsonAsync(Game.Api($"feedback/{id}/comments"), new { content = "补充信息" });

        var detail = await owner.GetFromJsonAsync<JsonElement>(Game.Api($"feedback/{id}"));
    ...
    public async Task Foreign_or_missing_feedback_reads_as_404()
    {
        ...
        Assert.Equal(HttpStatusCode.NotFound, (await other.GetAsync(Game.Api($"feedback/{id}"))).StatusCode);
        Assert.Equal(HttpStatusCode.NotFound, (await owner.GetAsync(Game.Api("feedback/999999"))).StatusCode);

        var foreignComment = await other.PostAsJsonAsync(Game.Api($"feedback/{id}/comments"), new { content = "越权" });
    ...
    public async Task Comment_length_is_validated()
    {
        ...
        var oversized = await owner.PostAsJsonAsync(Game.Api($"feedback/{id}/comments"),
            new { content = new string('x', 5001) });
        Assert.Equal(HttpStatusCode.BadRequest, oversized.StatusCode);

        var empty = await owner.PostAsJsonAsync(Game.Api($"feedback/{id}/comments"), new { content = "" });
    ...
    public async Task Comments_are_rate_limited_per_player()
    {
        ...
        for (var i = 0; i < 3; i++)
        {
            var created = await client.PostAsJsonAsync(Game.Api($"feedback/{id}/comments"), new { content = $"c{i}" });
            Assert.Equal(HttpStatusCode.Created, created.StatusCode);
        }
        var limited = await client.PostAsJsonAsync(Game.Api($"feedback/{id}/comments"), new { content = "c4" });
