...
The player side of the flow ships as a Godot addon in this repository (`addons/gd_feedback/`): it obtains the Steam ticket from the game's own Steam integration, exchanges it for an access token, and submits Feedback and Feedback Comments. The addon is game-agnostic and needs no per-Game code: its base URL is just the feedback service address, and the `/g/{appId}` path segment comes from the host — either from `SteamAppId` in the host's config resource (`feedback_config.tres`, or its project-root short alias `feedback.tres`), or from the AppID the host injects through `IGameAppIdProvider` when it is only known at runtime (the configured value wins when both are set). The addon never reads Steam directly, so the host supplying the value is what keeps its core engine-free and dependency-free (ADR-0004).
